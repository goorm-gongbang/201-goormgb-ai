...
- Grafana 보드 방향과 Discord webhook 방향을 한 문서에서 같이 보는 편이 운영 기준이 더 명확하다.
- `36`과 `37`이 별도 authority로 남아 있으면 범위와 KPI 해석이 다시 겹친다.
- 라벨 데이터가 필요한 KPI를 이번 범위에서 제외하면서,
  남는 핵심은 `post-review 결과 상황판 + webhook`으로 수렴했다.
