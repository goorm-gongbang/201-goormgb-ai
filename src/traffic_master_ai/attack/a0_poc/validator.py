...
class EventValidator:
    ...
    def validate_schema(self, event: SemanticEvent) -> ValidationResult:
        ...
        # event_type 검증
        if not event.type.value:
            errors.append("event_type is required")
        elif not self._is_valid_event_type(event.type.value):
            errors.append(f"unknown event_type: {event.type.value}")
        ...
        if errors:
            return ValidationResult.failure(errors, event.type.value)
        return ValidationResult.success(event.type.value)
    ...
    def validate_state_validity(
        ...
        try:
            event_type_enum = EventType(event.type.value)
        except ValueError:
            # 알 수 없는 event_type - schema validation에서 처리
            errors.append(f"unknown event_type for state-validity: {event.type.value}")
            return ValidationResult.failure(errors, event.type.value)
        ...
        if not is_valid_in_state(event_type_enum, current_state):
            ...
            errors.append(
                f"event '{event.type.value}' is not valid in state {current_state.value}. "
    ...
    def validate(
        ...
        *,
        ...
        if all_errors:
            ...
            return ValidationResult.failure(all_errors, event.type.value)
