...
class TestFailureMatrix:
    ...
    def test_seat_taken_policy(self, matrix: FailureMatrix) -> None:
        ...
        policy = matrix.get_policy(State.S5_SEAT, EventType.SEAT_TAKEN)
        assert policy is not None
        assert policy.failure_code == FailureCode.F_SEAT_TAKEN
        assert policy.recover_path == State.S5_SEAT
    ...
    def test_challenge_failed_policy(self, matrix: FailureMatrix) -> None:
        ...
        policy = matrix.get_policy(State.S3_SECURITY, EventType.CHALLENGE_FAILED)
        assert policy is not None
        assert policy.failure_code == FailureCode.F_CHALLENGE_FAILED
        assert policy.recover_path == State.S3_SECURITY
    ...
    def test_timeout_self_path(self, matrix: FailureMatrix) -> None:
        ...
        # S4에서 타임아웃
        policy_s4 = matrix.get_policy(State.S4_SECTION, EventType.TIMEOUT)
        assert policy_s4 is not None
        assert policy_s4.recover_path == State.S4_SECTION

        # S1에서 타임아웃
        policy_s1 = matrix.get_policy(State.S1_PRE_ENTRY, EventType.TIMEOUT)
        assert policy_s1 is not None
        assert policy_s1.recover_path == State.S1_PRE_ENTRY

    def test_session_expired_rollback_to_s0(self, matrix: FailureMatrix) -> None:
        """세션 만료 시 어느 상태에서나 S0로 가는지 검증."""
        for state in [State.S2_QUEUE_ENTRY, State.S5_SEAT, State.S6_TRANSACTION]:
            policy = matrix.get_policy(state, EventType.SESSION_EXPIRED)
            assert policy is not None
            assert policy.recover_path == State.S0_INIT
    ...
    def test_unknown_event_returns_none(self, matrix: FailureMatrix) -> None:
        ...
        policy = matrix.get_policy(State.S4_SECTION, EventType.SECTION_SELECTED)
        assert policy is None

    def test_payment_timeout_leads_to_sx(self, matrix: FailureMatrix) -> None:
        """결제 타임아웃(S6) 시 SX로 종료되는지 확인."""
        policy = matrix.get_policy(State.S6_TRANSACTION, EventType.PAYMENT_TIMEOUT)
        assert policy is not None
        assert policy.recover_path == State.SX_TERMINAL
